...
@section('content')
    <div class="container">
        ...
        @component('admin.components.breadcrumb')
            @slot('title') Список ползователей @endslot
        ...
        <table class="'table table-striped">
            ...
            @forelse ($users as $user)
                <tr>
                    ...
                    <td>{{$user->email}}</td>
                    <td class="text-right">
                        <form onsubmit="if(confirm('Удалить?')){ return true }else{ return false }" action="
                            {{route('admin.user_managment.user.destroy', $user)}}" method="post">
                            {{ mathod_field('DELETE') }}
                            {{ csrf_field() }}
